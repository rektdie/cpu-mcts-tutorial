...
fn iteration(position: &mut ChessPosition, tree: &mut Tree) {
    ...
    while tree[current_node].children.len() > 0 {
        current_node = select(current_node, tree, current_node == 0);
        selection_stack.push(current_node);
...
fn select(parent_idx: usize, tree: &Tree, root: bool) -> usize {
    const ROOT_C: f32 = 1.91;
    const C: f32 = 1.41;
    ...
    for &child_idx in parent_node.children.iter() {
        ...
        let score = q + cpuct * p * expl_factor;
